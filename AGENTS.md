...
Stage 5D rule governance is owner-approved only for the exact `stage5_synthetic_execution_validation` scope. Preserve the 48-item specification and zero-authority draft machine proposal byte-for-byte; the separate approved bundle, approval record, approval document, and `stage5d_governance.py` may issue only the exact anonymous synthetic `research` validation capability. The approval lineage is not a business implementation. Implement and independently accept `5D-1` pure in-memory Ledger V2, corporate actions, marks/NAV, two-dimensional P&L attribution, and complete replay before `5D-2`; durable persistence remains `currently_authorized=false` until that gate passes. SQLite schema v4, migrations, persistence, backtest, paper, shadow, live, real positions/accounts/orders, broker connectivity, funds deployment, and KB writes remain unauthorized. Preserve Stage 5A and Stage 5C historical artifacts unchanged.
