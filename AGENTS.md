...
As of 2026-08-11, Stage 2A/2B passed re-entry review and Stage 3 has resumed. Stage 3A completed offline Client/export contract validation at KB commit `2c84277ef463b5dd9a3fda3f2976a30cade53af5`. Stage 3B repinned the transport snapshot to KB commit `aab36fe229104779b50ec71e2dc37a9fad81d285` and passed independent-process loopback HTTP compatibility with read-only credentials. Stage 3C then passed real tcloud public-HTTPS transport validation for the exact formal `market_daily` Release, Manifest, Status and artifact with read-only credentials; 3B—3C grant no current-status authority and issued no `RunReleaseStatusConfirmation`. Stage 3D still requires an exact formally published Context Pack strategy smoke. Never use a KB worktree, internal package, mock, or same-process TestClient as evidence for 3B—3D. Stage 4 is `completed_with_scope_limits`: 4A-1—4A-4 and the independent 4B complete orchestration are implemented only in their exact approved `stage4_synthetic_research_validation` scope. The 4B engine must recompute every local result from raw typed case inputs; caller-supplied partial results, real KB Releases, KB current-status authority, real/executable prices, and real holdings remain forbidden. Never reuse the Stage 2B capability or a local 4A capability as the 4B capability, and do not implement PRD `hypothesis`/`TBD` values as defaults.
