...
As of 2026-08-08, Stage 2A/2B passed re-entry review and Stage 3 has resumed. Stage 3A pinned KB commit `2c84277ef463b5dd9a3fda3f2976a30cade53af5` and completed offline Client/export contract validation; it grants no current-status authority. Stage 3B requires an independent KB RC HTTP process and read-only credentials, Stage 3C requires a separately validated tcloud endpoint, and Stage 3D requires an exact formally published Context Pack Release. Never use a KB worktree, internal package, mock, or same-process TestClient as evidence for 3B—3D. Stage 4 remains in `4A rule governance`: 4A-1—4A-3 are implemented only in their approved synthetic scope, while 4A-4 remains a zero-authority draft. Never reuse the Stage 2B capability for Stage 4, and do not implement PRD `hypothesis`/`TBD` values as defaults. Backtest, paper, shadow, live, positions, and orders remain unauthorized.
