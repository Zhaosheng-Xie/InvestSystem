...
Stage 5A rule governance is owner-approved only for the exact `stage5_synthetic_execution_validation` scope. Preserve the original specification and zero-authority draft proposal unchanged; the separate approved bundle, approval record, and `stage5_governance.py` may issue only the exact anonymous synthetic `research` validation capability. Stage 5B—5D business evaluators, historical market-rule tables, fills, portfolio state, ledger, P&L, replay engine, SQLite migrations, and persistence are not implemented yet. Backtest, paper, shadow, live, real positions/accounts/orders, broker connectivity, funds deployment, and KB writes remain unauthorized.
