...
Stage 5D is in exact-rule governance only. Its 48-item specification and draft machine proposal are zero-authority and await explicit owner approval. Do not implement Stage 5D business evaluators, Ledger V2 changes, SQLite schema v4, migrations, persistence, approved bundles, approval records, or runtime capability before all 48 items are approved. If approved, implement `5D-1` pure in-memory corporate actions, marks/NAV, two-dimensional P&L attribution, and complete replay before `5D-2` durable SQLite atomic persistence; preserve Stage 5A and Stage 5C historical artifacts unchanged.
