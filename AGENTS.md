...
Stage 5D rule governance is owner-approved only for the exact `stage5_synthetic_execution_validation` scope. Preserve the 48-item specification and zero-authority draft machine proposal byte-for-byte; the separate approved bundle, approval record, approval document, and `stage5d_governance.py` may issue only the exact anonymous synthetic `research` validation capability. The approval lineage is not a business implementation. The current `5D-1` engineering acceptance scope is deliberately bounded to the first pre-registered order/contract historical replay: implement only the source-driven Ledger V2, settlement/availability, costs, actually encountered corporate-action semantics, marks/NAV, P&L and deterministic complete replay required by that exact case and horizon. The exact case, horizon, event inventory, support matrix and accounting golden are frozen in `docs/validation/stage5d-first-order-contract-replay-preregistration.md` and its machine fixture; do not change them after observing implementation results. The 48 approved items remain the long-term governance ceiling, but full securities-accounting coverage is not a completion requirement for this bounded slice. Any unsupported accounting or corporate-action input must fail closed as `BLOCKED`/`ABSTAIN` before publishing partial NAV/P&L; never silently ignore, approximate or post-select it. Accept the bounded `5D-1` slice independently before discussing a separately authorized minimum `5D-2`; durable persistence remains `currently_authorized=false`. SQLite schema v4, migrations, persistence, backtest, paper, shadow, live, real positions/accounts/orders, broker connectivity, funds deployment, and KB writes remain unauthorized. Preserve Stage 5A and Stage 5C historical artifacts unchanged.
