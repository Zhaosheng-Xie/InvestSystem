...
Stage 5 has entered 5A rule governance only. `Stage5_5A成交组合账本与确定性回放精确规则包_v0.1.md` and its `0.1.0-draft` machine proposal contain 40 pending owner decisions, empty allowed run modes, no approval record, no capability, and no evaluator. Do not implement or default any proposed execution, participation, cost, risk, drawdown, ledger, corporate-action, P&L, or replay business rule until the owner explicitly approves the whole package and separate approved artifacts exist. Backtest, paper, shadow, live, real positions/accounts/orders, broker connectivity, and KB writes remain unauthorized.
