...
| Stage 6 | 6B 验收；6C v0.2 批准谱系、[统一 synthetic phase seal 验收](docs/validation/stage6c-synthetic-phase-seal-acceptance.md)及[正式执行 Readiness 审计](docs/validation/stage6c-formal-execution-readiness-audit-v0.1.md)；审计为 `9 READY / 22 MISSING / 1 BLOCKED / 2 N/R` | 正式 execution authorization/admission/migration、2019—2025 PIT readiness、EXIT/SELL 与 Stage 5D/peer support、real experiment/audit/custody 和 6D | `in_progress / synthetic kernel completed_with_scope_limits / formal execution blocked` |
...
3. Stage 6A/6B 与真实 validation-only seal 已完成；6C v0.2 anonymous synthetic kernel 与统一 phase seal 已验收。[正式 Readiness 审计](docs/validation/stage6c-formal-execution-readiness-audit-v0.1.md)结论为 `NO_GO`。下一步并行完成 KB 公共交付 census 和 IS outcome-blind Stage 5D support census，再由 owner 决定最小 EXIT/SELL 生命周期、正式 migration 和 holdout custodian；不得先启动正式执行或继续堆叠 synthetic 切片。
