...
| Stage 5 | `in_progress / 5B—5C completed_with_scope_limits` | 成交、组合与确定性回放 | 5A 治理、5B 历史市场/合成成交和 5C 合成组合/内存账本已完成；5D 未开始 |
...
状态：`in_progress / 5A governance approved / 5B—5C completed_with_scope_limits / 5D not_started`
...
1. 是否开始 Stage 5D 的非空公司行动、marks、NAV/P&L、外部现金流、完整 Stage 5 replay/golden matrix 与 SQLite durable atomic persistence；其 scope 仍只能是 `stage5_synthetic_execution_validation`。
2. Stage 2B/4B 返回的 Manifest、Replay 和 DecisionRecord 是否与 Stage 5D 账本制品一并进入 durable SQLite，以及对应原子性、幂等、冲突、migration 和失败语义。
...
| Stage 5 | [5A 精确规则草案](产业卡点及事件驱动系统/03_规则与规格/Stage5_5A成交组合账本与确定性回放精确规则包_v0.1.md)、[批准记录](产业卡点及事件驱动系统/03_规则与规格/Stage5_5A成交组合账本与确定性回放批准记录_v0.1.md)、approved bundle、approval record、capability guard、[5A 治理验收](docs/validation/stage5-5a-governance-acceptance.md)、[5B 市场/成交验收](docs/validation/stage5-5b-market-execution-acceptance.md)与[5C 组合/账本验收](docs/validation/stage5-5c-portfolio-ledger-acceptance.md)；历史规则、首次可成交、成本/冲击、deterministic synthetic fill、组合风险/五层数量、提交前现金与可卖量、结算投影和内存 append-only 双分录账本已实现，零真实/交易权限 | 5D 公司行动、NAV/P&L、SQLite durable atomic persistence、完整 replay/golden matrix 实现与合成验收 | `in_progress / 5B—5C_completed_with_scope_limits` |
...
3. Stage 5A 四十项已批准，5B 市场/成交与 5C 组合/内存账本已完成 scope-limited 合成验收；下一步是 5D 公司行动、NAV/P&L、完整 replay 和 durable persistence，不得跨批把预留事件或未实现能力写成完成。
