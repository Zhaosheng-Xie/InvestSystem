...
- Stage 3A 已在 KB RC 契约提交 `2c84277ef463b5dd9a3fda3f2976a30cade53af5` 完成只读 HTTP Client、不可变导出包验证器和官方 fixture 离线验收。Stage 3B 随后把完整 Stage 6B transport snapshot 重固定到 `aab36fe229104779b50ec71e2dc37a9fad81d285`，并通过独立 KB RC 进程、短期只读凭据的真实本机 HTTP 兼容验收。Stage 3C 又以相同固定 Client/契约和新的短期只读凭据，通过真实公网 `https://82.157.112.120` 获取正式 `market_daily` Release、Manifest、Status 与 artifact 并闭合全部响应/制品身份。Stage 3D 已消费正式 Context Pack 与 Evidence Release，完成 provider-neutral 映射、IS 自有四制品 Receipt/三类 Observation、validation-only StrategyRunManifest 和真实只读策略入口 smoke；因规则未授权真实策略结论且材料显式不完整，结果正确为 `ABSTAIN`。3B—3D 都没有签发 `RunReleaseStatusConfirmation`，所有输出仍为 `authority_eligible=false`；新 run authority、持久化准入、完整产业策略集成和任何真实运行模式仍不存在。
...
- owner 已批准 [Stage 5 / 5A 成交、组合、账本与确定性回放精确规则包](产业卡点及事件驱动系统/03_规则与规格/Stage5_5A成交组合账本与确定性回放精确规则包_v0.1.md)第 13 节全部四十项，且只授权 `stage5_synthetic_execution_validation`。原规格和零权限 draft proposal 保持不变；独立[批准记录](产业卡点及事件驱动系统/03_规则与规格/Stage5_5A成交组合账本与确定性回放批准记录_v0.1.md)、approved bundle、approval record 和治理 capability verifier 已形成。Stage 5B 已完成历史市场与 synthetic fill 纵向切片并通过[验收](docs/validation/stage5-5b-market-execution-acceptance.md)；Stage 5C 已完成合成组合、受约束成交与内存账本纵向切片并通过[验收](docs/validation/stage5-5c-portfolio-ledger-acceptance.md)。
- owner 已原子批准 [Stage 5D 公司行动、估值、P&L、完整回放与原子持久化精确规则](产业卡点及事件驱动系统/03_规则与规格/Stage5_5D公司行动估值P&L完整回放与原子持久化精确规则包_v0.1.md)第 13 节全部 48 项，且仍只授权 `stage5_synthetic_execution_validation`。原规则和零权限 draft machine proposal 保持原始字节不变；独立批准谱系精确固定 Stage 5A/5C 上游。owner 于 `2026-08-12` 将 5D-1 工程完成口径收窄为第一条预注册“订单/合同事件”历史回放；[预注册](docs/validation/stage5d-first-order-contract-replay-preregistration.md)现已按[受限验收](docs/validation/stage5d-first-order-contract-replay-acceptance.md)闭合同次 Stage 5C 重算、期初 opening 重物化、五事件 Ledger V2、mark/NAV、十八格 P&L 和 complete/audit replay。48 项继续作为长期治理上限，未覆盖场景 fail closed。5D-2 persistence 仍等待另行授权，所有真实/交易模式继续关闭。
...
- `2026-07-30` 复核时，KB Stage 6A provider 与公共契约基线为 `58ed9c5cb5302e3e719f1696bed83a03c5d6313b`，当时公网只交付 16 条 `market-daily` 样本。到 `2026-08-11`，Stage 3D handoff 已通过正式发布面提供并验收 Context Pack Release `rel_fc8be9b554aa414ca8ad5a14aaec69d9` 与 Evidence Release `rel_02f1148031c04036a5f7d5cda9807fb5`；这只证明本次 Context Pack 承重闭包已公开交付，不应外推为 KB 全部财务/研究数据都已公网部署。异机灾备副本仍属于 KB 运维边界，不是 InvestSystem 的 Stage 3D 依赖。
...
| Stage 6 | `in_progress / 6B completed_with_scope_limits / 6C synthetic kernel completed_with_scope_limits` | 历史验证与冠军挑战 | 六条匿名 synthetic 切片及统一 phase seal 已验收；seal 强制 not-ready-for-6D，正式 run、migration、真实 evidence/custody 与 6D 未授权 |
...
- **3C / completed**：以相同只读 Client、`aab36fe` 固定 transport snapshot 和新的短期只读凭据连接真实公网 `https://82.157.112.120`，获取正式 `market_daily` Release `rel_10e257ad87734d7bb5cadc55e7b444e7`、Manifest、完整 Status 与 artifact `market-daily-v1`。三类响应哈希、`published` 状态、knowledge cutoff、artifact ID/大小/SHA-256 及 `Content-Length`、`ETag`、`X-Artifact-ID`、`X-Artifact-SHA256`、`X-Dataset-Release-ID` 全部闭合；结果仍为 `authority_eligible=false`，未签发确认或写入业务状态。证据见 [Stage 3C 真实 tcloud 只读 HTTPS 验收](docs/validation/stage3c-tcloud-http-acceptance.md)。
- **3D / completed_with_scope_limits**：使用固定 `aab36fe` transport snapshot 和真实公网 HTTPS 获取 Context Pack Release `rel_fc8be9b554aa414ca8ad5a14aaec69d9`、Evidence Release `rel_02f1148031c04036a5f7d5cda9807fb5`、两份 Manifest/Status、两份主制品和两份公开 Schema；验证查询/制品等值、引用闭包、PIT、自哈希与响应头。IS 独立生成 provider-neutral 输入、四制品 Receipt、三类只读 Observation 和 validation-only StrategyRunManifest；策略入口因真实规则未授权且存在声明缺口而正确 `ABSTAIN`。没有签发 `RunReleaseStatusConfirmation`、写入状态或获得 run authority。证据见 [Stage 3D 真实公网 Context Pack 验收](docs/validation/stage3d-context-pack-http-acceptance.md)。
...
状态：`in_progress / 5A governance approved / 5B—5D-1 completed_with_scope_limits`

5A 完成记录：owner 于 `2026-08-08` 批准[Stage 5 / 5A 成交、组合、账本与确定性回放精确规则包](产业卡点及事件驱动系统/03_规则与规格/Stage5_5A成交组合账本与确定性回放精确规则包_v0.1.md)第 13 节全部四十项，只授权 `stage5_synthetic_execution_validation`。原规格与 draft machine proposal 保持不可变；新的[批准记录](产业卡点及事件驱动系统/03_规则与规格/Stage5_5A成交组合账本与确定性回放批准记录_v0.1.md)、approved bundle 和 approval record 精确固定 Stage 4B 上游身份、市场/成本/风险/账本/P&L/replay 语义及零真实权限。

5B 完成记录：`stage5_governance.py` 新增不可伪造的市场执行 typed capability；`stage5_market_execution.py` 实现历史有效 `MarketRuleSet`、版本化交易日历、证券会话状态、非可成交提案参考价、四类市场数据、首个可成交窗口、lot/tick/5% 容量、历史成本表、单调冲击曲线、可成交价 Gate 3/4 重算、三日入场到期、`DAY` intent、确定性 partial/full synthetic fill、余量取消和 replay。Stage 5B 要求同一运行的原始 Stage 4 case 与 result/input/replay 精确绑定，避免从 Stage 4 汇总指标倒推估值输入。正式证据见 [Stage 5B 验收记录](docs/validation/stage5-5b-market-execution-acceptance.md)。该验收当时明确不包含组合、账户与账本；这些能力不能回溯冒充为 5B 交付。

5C 完成记录：新增合成账户/风险/恢复/结算 typed contracts、精确组合风险与五层数量计算、Stage 5B candidate → reduction constraint → finalize 两阶段接缝、现金/成本准备金/可卖量的提交前约束，以及截至 injected clock 的内存 append-only 双分录账本、FIFO lot、结算与证券可用性投影、reversal/replacement 和确定性 partial replay。5C 重新绑定原始 5B case/result/input/replay，受约束数量只能减少；未获组合批准或未提交的数量不会冒充订单取消，未来结算事件也不会泄漏到当前状态。正式证据见 [Stage 5C 验收记录](docs/validation/stage5-5c-portfolio-ledger-acceptance.md)。非空公司行动、marks、NAV/P&L、外部现金流、SQLite migration、durable atomic persistence、完整 Stage 5 replay 与 golden matrix 均留在 5D。

5D 治理与实施记录：Stage 5A 的原则级文字不足以唯一决定费用/税费/滑点与 realized/unrealized 的加总关系、五类公司行动的权益和舍入、mark/NAV 的 PIT，以及 SQLite 的原子对象集合，因此形成 48 项[精确规则](产业卡点及事件驱动系统/03_规则与规格/Stage5_5D公司行动估值P&L完整回放与原子持久化精确规则包_v0.1.md)。owner 已整包原子批准，原 draft bytes 保持不变；独立批准谱系精确绑定全部 48 项、Stage 5A approved identity 与 Stage 5C commit `7f64c584c5c7be5e2385a177fab9e5d31e3f665b`。[第一条预注册订单/合同历史回放](docs/validation/stage5d-first-order-contract-replay-preregistration.md)现已完成[受限验收](docs/validation/stage5d-first-order-contract-replay-acceptance.md)：固定 `stage4_4a4_case_001` 匿名合成 ENTER/BUY，按 `(2025-01-20T02:10:00Z, 2025-01-21T07:01:00Z]` 重放 opening、buy、现金结算、证券交收、可卖五类事件；期末 mark `8`、NAV `99986.77`、十八格总 P&L `-13.23`，complete replay SHA-256 为 `f5ed17d1bf9944d35b7a5afa36e68df0fc40d12c277874ea01d02d1e0bd59225`。其他规则保持批准但未实现，越界输入显式失败关闭。`5D-2` 仍为 `currently_authorized=false`。
...
- 对第一条预注册订单/合同 case/horizon 先形成完整事件清单，只实现其中实际出现的历史规则、公司行动、盘后公告和不可成交语义；不得按回放结果事后挑选窗口。
- 建立风险簇、现金、单票/组合风险预算和容量约束。
- 分离 target、approved、submitted、filled 和 actual 仓位。
- 实现退出、降险、停机和合成执行账本；paper 运行仍需后续独立批准。
- 建立该受限 case 的成交、组合、最小 mark/NAV、P&L 和 complete replay；未支持事件在任何金融输出产生前 fail closed。

完成门：

- 第一条预注册订单/合同历史回放中实际出现的市场边界、规则切换、公司行动、盘后事件和不可成交情形通过 golden/replay 测试；全量证券会计覆盖不作为本切片完成门。
- 该切片内任一成交、现金/证券变化、NAV 与 P&L 都能追溯到决策、批准、市场数据、承重事件和规则版本。
- 支持矩阵以外的输入稳定返回 `BLOCKED/ABSTAIN` 且不发布 partial NAV/P&L；不得把未覆盖场景从样本中静默删除。
...
状态：`in_progress / 6B completed_with_scope_limits；6C synthetic kernel completed_with_scope_limits；formal execution readiness blocked；6D not authorized`
...
owner 已原子批准[6B 历史准入、状态确认与原子留存精确规则](产业卡点及事件驱动系统/03_规则与规格/Stage6_6B历史准入状态确认与原子留存精确规则包_v0.1.md)第 14 节全部 32 项；原 specification 与零权限 draft machine proposal 保持原始字节，独立[批准记录](产业卡点及事件驱动系统/03_规则与规格/Stage6_6B历史准入状态确认与原子留存批准记录_v0.1.md)、approved bundle、approval record 和 fail-closed validation capability verifier 已形成。该 capability 只允许实现和验收隔离临时库中的 admission contracts、真实只读 HTTPS status confirmation 与 validation-only seal；正式历史 run、策略 evaluator、正式状态库 migration、6C/6D 和全部交易权限继续关闭。

[6B 离线实现验收](docs/validation/stage6b-historical-admission-offline-acceptance.md)已闭合纯 contracts、受信 evidence issuer、KB provider status adapter、根级 HTTP/封存编排、隔离 validation state/cache、单事务 seal-last、完整闭包/CAS 重验、失败零写、并发幂等和架构隔离。[真实 HTTPS validation-only seal 验收](docs/validation/stage6b-live-https-acceptance.md)随后使用专用 producer handoff 与短期只读凭据，独立获取根和 Source Release 的 Release/Manifest/Status 及全部十个 artifact，并在新的状态窗口内完成隔离 confirmation/seal。全仓为 `1018 passed, 4 skipped`，所有输出仍为 `authority_eligible=false` 且 evaluator 调用为零；正式 historical run、正式 migration 与 6C/6D 继续关闭。

[6C 开发样本、Walk-Forward 与冠军挑战精确规则草案](产业卡点及事件驱动系统/03_规则与规格/Stage6_6C开发样本WalkForward与冠军挑战精确规则包_v0.1.md)已形成 40 项待批准决定，并通过[草案形成验收](docs/validation/stage6c-development-walk-forward-draft-acceptance.md)：固定 2019—2021 development、2022—2025 四个 walk-forward folds、60-session 主 horizon、coverage/样本/统计/压力/冠军门和未打开的 2026 holdout。草案明确当前受限 5D-1 不满足正式 6C，且 6B validation-only seal 不是正式 run authority；没有 approved bundle、approval record、evaluator 或任何 holdout 访问。

方法复核发现 v0.1 的 holdout 仅逻辑隔离、adjusted inference 未进入同一完成门、portfolio/bootstrap 层次不唯一、coverage 无法排除可观察选择偏差及 TWR/benchmark 公式不完整。[6C v0.2 修订草案](产业卡点及事件驱动系统/03_规则与规格/Stage6_6C开发样本WalkForward与冠军挑战精确规则包_v0.2.md)及[修订验收](docs/validation/stage6c-v02-methodology-revision-acceptance.md)已集中关闭这五项；v0.1 保留为 traceability-only。v0.2 修订形成时 40 项全部 pending，且没有 approved bundle、capability 或 runtime。
...
[6C candidate/fold/coverage 第二切片](docs/validation/stage6c-candidate-fold-coverage-slice-acceptance.md)已完成 outcome-blind candidate inventory、2022—2025 fold planner、120-session label/purge、20-session embargo、总/年度 coverage、continuous SMD 与 material categorical balance；40-candidate golden 为总体 `0.8`、四年各 `0.75`、replay `09ad4af…7202`。晚 2025 label 仍留在 coverage 分母但不进入 evaluation，防止 holdout 泄漏。

[6C bootstrap/cluster/Holm 第三切片](docs/validation/stage6c-inference-holm-slice-acceptance.md)已完成 source-recomputed daily calendar-block bootstrap、company/risk paired-contribution wild bootstrap 与五项独立模型 TWR source 的 Holm family；三组固定 seed/10,000 draws 均可重放。匿名 golden inference replay 为 `c4c46136…3b4b`，Holm replay 为 `ce0c8738…b383`，但只证明统计 kernel 公式与失败语义。

[6C synthetic champion 第四切片](docs/validation/stage6c-synthetic-champion-gate-acceptance.md)统一重算前三切片并实现 completed-trade/per-fold、3/4 fold、worst fold、2pp material increment、15% drawdown、25% winner、1.5× friction、Holm 与 P0 audit 门；golden 32 trades、fold `5/4/3/-5`、drawdown `0.05`，replay `37ef9941…b6b9`。结果严格标记 `not_a_real_6d_request=true`。

[6C peer/experiment 第五切片](docs/validation/stage6c-peer-experiment-ledger-slice-acceptance.md)实现 target-excluded PIT peers、三层 fallback、最少五 peers、exact `1/N` 权重，以及 8 ablation + 16 stress 的 pre-performance registration/outcome ledger；32-basket set hash 为 `bce797bf…b011`，ledger hash 为 `33c75ebd…30b3`。这些仍是匿名 contract evidence。

[6C synthetic phase 第六切片](docs/validation/stage6c-synthetic-phase-seal-acceptance.md)将前五切片 raw sources 放入单一 content-addressed bundle，依次重建 peer set、experiment ledger 和 champion gate 后生成 `SYNTHETIC_PHASE_SEALED`；source hash `6b04d62a…5754`，seal hash `b9f8f41d…bf5f`。seal 强制 `not_ready_for_6d_freeze=true`、zero I/O/persistence/authority。

[6C 正式执行 Readiness 审计 v0.1](docs/validation/stage6c-formal-execution-readiness-audit-v0.1.md)在 synthetic kernel 收口后检查了 34 个正式前置门：`9 READY / 22 MISSING / 1 BLOCKED / 2 NOT_REQUIRED_WITH_JUSTIFICATION`。结论为 `NO_GO_FOR_FORMAL_STAGE6C_EXECUTION`；Critical 缺口是 2019—2025 历史 PIT 总体证据和能够产生已结束交易的 EXIT/SELL 生命周期。下一动作限定为 KB 公共交付 census、IS outcome-blind Stage 5D support census，以及正式 migration/holdout custody 方案，不启动历史运行。
...
[完整生命周期承重输入物化验收](docs/validation/stage5d-normal-lifecycle-input-materialization-acceptance.md)现已冻结 full-EXIT raw Stage 5C case、60-session calendar、59-mark set/coverage、validation-horizon exit mandate 与 rule/cost/impact/settlement closure 六个非空 hash；完整 input-set hash 为 `60cad843…9e30`。既有 Stage 5C 校验确认 `SELL 200 @ 7.96`、净回款 `1585.18`、期末数量零，但这只证明输入可执行；lifecycle evaluator 不存在，全部 result/replay hashes 仍为 `null`。
...
[Stage 6 最小历史公共数据消费契约 v0.2](docs/validation/stage6-minimum-public-data-consumption-contract-v0.2.md)已获 owner 原子批准：`S6DATA-01—10` 全部生效，Beta benchmark 精确固定为 `H00985`，其余采用 v0.1 推荐的 SSE/SZSE universe、ADV20/Beta120、Stage 5D 五类公司行动、三个 source families + 单一 root aggregate Release、11 个 artifact Schema 和 repin 时点。该批准只形成需求治理和审批谱系，仍为零 runtime authority；v0.1 draft 保持原始字节供追溯。

[ADR-0002 提议](docs/adr/ADR-0002-kb-provider-contract-consumer-profile-boundary.md)和 [Stage 6 历史公共数据消费与验收 Profile v0.3 草案](docs/validation/stage6-historical-public-data-consumer-profile-v0.3-draft.md)现将 v0.2 重新分层为 IS 消费需求与验收口径：KB 只提供通用 Release、事实、benchmark/factor、dependency closure 和质量 Profile；IS Adapter 构造 `StrategyInputRef` 并持有 H00985 选择、ADV20/Beta120 接受、holdout、candidate、coverage 与 authority。`S6BOUND-01—10` 全部 pending，草案不批准 KB Schema 实现、IS parser/repin 或任何数据运行。
...
- 在查看策略表现前预注册 Stage 5D 支持矩阵与事件 inventory；样本命中未支持会计事件时保留为 `BLOCKED/ABSTAIN`，不得事后剔除或扩张实现来美化结果。
...
6. Stage 5D 第 13 节全部 48 项已原子批准并继续作为长期治理上限；当前 5D-1 工程门收口为第一条预注册订单/合同历史回放所需的纯内存闭环，未覆盖场景一律失败关闭。该受限 5D-1 独立验收后才可讨论并另行授权最小 5D-2 持久化；全部真实/交易权限保持关闭。

### 11.3 后续需要用户确认

1. Stage 8 是否长期停在 shadow/paper，还是未来允许另行评估人工批准的小额 canary。
...
| Stage 6 | 6B/6C synthetic 验收、Readiness、Stage 5D normal lifecycle、[KB handoff runbook](docs/validation/stage6-kb-historical-handoff-acceptance-runbook-v0.1.md)、已批准[最小消费契约 v0.2](docs/validation/stage6-minimum-public-data-consumption-contract-v0.2.md)及待批准[消费者 Profile v0.3](docs/validation/stage6-historical-public-data-consumer-profile-v0.3-draft.md)；KB 声明当前 NOT_READY 且无 eligible Release | owner 审核 KB 通用提供方/IS 消费者边界；随后 KB 通用 Schema/catalog/fixture 与 IS Adapter 离线映射；再另行授权数据源、backfill、RC、repin、真实 PIT/candidate coverage、formal admission 和 6D | `in_progress / consumer boundary draft pending owner / awaiting generic KB contracts / formal execution blocked` |
...
3. Stage 6 synthetic 与 Stage 5D normal lifecycle 技术接缝已完成；KB 声明当前无 2019—2025 eligible Release。[最小消费契约 v0.2](docs/validation/stage6-minimum-public-data-consumption-contract-v0.2.md)继续原子批准 `S6DATA-01—10`，其中 Beta benchmark 固定为 `H00985`。在 owner 审核 ADR-0002/v0.3 前，不得把 v0.2 的 IS 内部字段继续物化为 KB 核心 Schema。边界批准后先由 KB 冻结通用 public contract/catalog/fixture，再由 IS 离线适配；数据源、backfill、RC、repin、parser、coverage、historical run 与 holdout 均须后续独立授权。
...
| `v2.6` | 2026-08-08 | `active` | 完成 Stage 5B 历史市场与合成成交纵向切片：精确绑定 Stage 5A capability 和同一运行 Stage 4 case/result/replay，实现历史规则/日历/会话、首次可成交、日线 VWAP、lot/tick/5% 容量、历史成本、单调冲击、当前价 Gate 重算、三日到期、DAY synthetic intent/fill/cancel 与 deterministic replay；全仓 `841 passed, 4 skipped`。5C—5D、SQLite、backtest/paper/shadow/live 与真实账户订单仍未实现或授权。 |
| `v2.7` | 2026-08-09 | `active` | 完成 Stage 5C 合成组合与内存账本纵向切片：实现内容寻址账户/风险/恢复/结算合同、风险预算和五层数量、两阶段 candidate/只减不增 constraint、提交前现金/成本准备金/可卖量、PIT 与承重证据哈希闭环、截至时点的 append-only 双分录/FIFO/结算投影及确定性 partial replay；Stage 5 专项 `93 passed`，全仓 `915 passed, 4 skipped`。5D 公司行动、NAV/P&L、SQLite、durable full replay 及全部真实/交易模式仍未实现或授权。 |
| `v2.8` | 2026-08-09 | `active` | 启动 Stage 5D 精确规则治理。因 Stage 5A 对 P&L 防双计、公司行动日期/权益/舍入、逐证券成本闭合、mark/NAV、完整 replay 和 SQLite 原子集合仍不足以唯一编码，形成 48 项待批准规则与零权限 draft machine proposal；冻结 exact event semantic map、十八格 P&L 公式、状态独立 PIT、`(beginning_at,ending_at]` 与同库 v4 原子边界，治理专项 `9 passed`、全仓 `924 passed, 4 skipped`。批准前不实现 evaluator、migration 或 runtime capability，全部真实/交易权限继续关闭。 |
...
| `v3.3` | 2026-08-12 | `active` | owner 将 Stage 5D 工程完成口径收敛为第一条预注册“订单/合同事件”历史回放：48 项 approved 规则继续作为长期治理上限且原文不变；5D-1 只实现该 case/horizon 实际承重的 source-driven Ledger V2、必要公司行动、mark/NAV、P&L 与 complete replay，未覆盖场景在金融输出前 `BLOCKED/ABSTAIN`。全量证券会计边角不再阻塞受限 5D-1 与后续受限 Stage 6；5D-2 和全部真实/交易权限仍须另行授权。 |
| `v3.4` | 2026-08-12 | `active` | 冻结 Stage 5D 第一条订单/合同回放预注册：固定 `stage4_4a4_case_001` 匿名合成 ENTER/BUY、`(2025-01-20T02:10:00Z, 2025-01-21T07:01:00Z]`、opening/buy/现金结算/证券交收/可卖五事件 closed world、期末 mark `8`、NAV `99986.77` 与总 P&L `-13.23`。机器预注册 canonical SHA-256 为 `f7042c49f72b693d1c9ae5892b1d454be07cf6ad0851c499b47b2fead55492bc`；预注册不是实现或验收，SELL、公司行动、外部现金流、SQLite 和全部真实权限继续关闭。 |
| `v3.5` | 2026-08-19 | `active` | 以 `completed_with_scope_limits` 关闭受限 5D-1：同次重算固定 Stage 5C case，将 opening 重物化到 period beginning，重放五事件 source-driven Ledger V2，物化 beginning/ending NAV、批准公式的十八格 P&L 和 deterministic complete/audit replay；固定 total P&L `-13.23` 与 complete replay SHA-256 `f5ed17d1…9225`。专项 `9 passed`、Stage 5D 相邻 `41 passed`、全仓 `964 passed, 4 skipped`；SELL、公司行动、外部现金流、5D-2 persistence 和全部真实/交易权限继续关闭。 |
| `v3.6` | 2026-08-19 | `active` | 从 `caf1e67` 建立 Stage 6 历史验证分支，形成 6A 预注册、PIT、支持矩阵、冠军挑战、统计与失败留存的 35 项零权限草案；PRD 数值门槛继续是 hypothesis，6B—6D 未授权。 |
| `v3.7` | 2026-08-19 | `active` | owner 原子批准 6A 全部 35 项；保留原 specification/draft，新增 exact approved bundle、approval record、批准记录与 fail-closed governance capability。该 capability 只允许形成 6B 待审批草案，不授权 historical run、confirmation、persistence、holdout、backtest 或交易。 |
| `v3.8` | 2026-08-19 | `active` | 形成 6B historical admission、真实 HTTPS current-status authority、完整 Release closure、run-scoped confirmation、原子 envelope/seal 和撤回 audit-only 的 32 项 exact draft。所有项目仍 pending，machine proposal 零 capability；不复用 Stage 5D SQLite v4，正式 migration 版本待 owner 决定，6C/6D 和全部历史/交易运行继续关闭。 |
| `v3.9` | 2026-08-19 | `active` | owner 原子批准 6B 全部 32 项；保留原 specification/draft，新增 exact approved bundle、approval record、批准记录与 fail-closed validation capability。该 capability 只允许隔离临时库中的 admission 实现、真实只读 HTTPS status validation、validation-only confirmation/seal；不授权正式 historical run、evaluator、正式 migration、6C/6D 或交易。 |
| `v3.10` | 2026-08-19 | `active` | 完成 6B contracts/issuer、provider status adapter、根级 HTTP 编排、隔离 validation state/cache、完整闭包原子 seal、失败矩阵和架构边界修复；离线实现验收为 `1014 passed, 4 skipped`。旧 Stage 3 Token 已撤销，6B 仍等待新短期只读凭据完成真实 HTTPS validation-only seal；正式 migration、historical run、6C/6D 和交易继续关闭。 |
| `v3.11` | 2026-08-20 | `active` | 完成 6B 真实公网 validation-only seal：专用 producer handoff 严格固定根与 Source Release 全闭包，只调用 Release/Manifest/Status/Artifact 公网面，下载并验证 10 个制品，在新的 Status 窗口内签发隔离 confirmation/seal；`authority_eligible=false`、策略 evaluator 调用为零、正式状态/缓存未修改。全仓 `1018 passed, 4 skipped`；正式 migration、historical run、6C/6D 和交易继续关闭。 |
...
| `v3.16` | 2026-08-20 | `active` | 完成 6C 第二匿名合成切片：outcome-blind candidate inventory、研究单位去重、2022—2025 folds、120-session purge、20-session embargo、holdout-label 排除、coverage 80%/70%、SMD 与 material categorical selection audit。收益/bootstrap/Holm/champion 和正式权限继续关闭。 |
| `v3.17` | 2026-08-20 | `active` | 完成 6C 第三匿名合成切片：内部重算 fold/coverage/TWR，calendar-quarter daily factor bootstrap、company/risk paired-contribution wild bootstrap、五项 Holm adjusted family 与完整 inference trail。champion/6D 和全部正式权限继续关闭。 |
| `v3.18` | 2026-08-20 | `active` | 完成 6C 第四匿名合成切片：typed fold/completion/friction/audit bindings，统一前三切片并实现 sample/fold/material/drawdown/winner/friction/Holm/P0 audit champion formula。只输出 synthetic gate 状态，不签发真实 6D 请求或任何正式权限。 |
| `v3.19` | 2026-08-20 | `active` | 完成 6C 第五匿名合成切片：PIT peer snapshot、target exclusion、三层 fallback、最少五 peers、exact 1/N 权重，以及 8 ablation/16 stress 的 pre-performance registration/outcome ledger。真实 peer returns、实验和统一 phase seal继续关闭。 |
...
| `v3.27` | 2026-08-24 | `active` | 基于 KB NOT_READY census 形成 Stage 6 最小消费契约草案：P0 历史证据/证券生命周期/mark-rule-state/actions/financials/peer factors，推荐 ADV20/Beta120 通用因子、三个 source families + 单一 root Release、artifact Schema v1 与 producer-validation 后 repin；十项 owner 决定保持 pending。 |
