...
- owner 已原子批准 [Stage 5D 公司行动、估值、P&L、完整回放与原子持久化精确规则](产业卡点及事件驱动系统/03_规则与规格/Stage5_5D公司行动估值P&L完整回放与原子持久化精确规则包_v0.1.md)第 13 节全部 48 项，且仍只授权 `stage5_synthetic_execution_validation`。原规则和零权限 draft machine proposal 保持原始字节不变；独立[批准记录](产业卡点及事件驱动系统/03_规则与规格/Stage5_5D公司行动估值P&L完整回放与原子持久化批准记录_v0.1.md)、approved bundle、approval record 和 fail-closed governance verifier 已形成并精确固定 Stage 5A/5C 上游。owner 于 `2026-08-12` 进一步收窄工程完成口径：5D-1 只须准确支持第一条预注册“订单/合同事件”历史回放及其实际出现的会计事件，不要求一次覆盖全部证券会计边角；48 项继续作为长期治理清单，未覆盖场景必须 fail closed。[首条回放预注册](docs/validation/stage5d-first-order-contract-replay-preregistration.md)现已冻结 case、horizon、五事件 inventory、零权限范围及价格不变的 `-13.23` P&L golden；这只是实施输入，不是 5D-1 验收。5D-2 persistence 仍等待该受限切片独立验收与另行授权，所有真实/交易模式继续关闭。
...
| Stage 5 | `in_progress / 5B—5C completed_with_scope_limits / 5D governance approved / 5D-1 bounded slice preregistered` | 成交、组合与确定性回放 | 5A 治理、5B 历史市场/合成成交和 5C 合成组合/内存账本已完成；5D 48 项规则及批准谱系已完成，第一条订单/合同历史回放的 case/horizon/事件 inventory 与会计 golden 已冻结，完整实现待开始 |
...
状态：`in_progress / 5A governance approved / 5B—5C completed_with_scope_limits / 5D governance approved / 5D-1 first_order_contract_replay preregistered`
...
5D 治理与实施记录：Stage 5A 的原则级文字不足以唯一决定费用/税费/滑点与 realized/unrealized 的加总关系、五类公司行动的权益和舍入、mark/NAV 的 PIT，以及 SQLite 的原子对象集合，因此形成 48 项[精确规则](产业卡点及事件驱动系统/03_规则与规格/Stage5_5D公司行动估值P&L完整回放与原子持久化精确规则包_v0.1.md)。owner 已整包原子批准，原 draft bytes 保持不变；新的[批准记录](产业卡点及事件驱动系统/03_规则与规格/Stage5_5D公司行动估值P&L完整回放与原子持久化批准记录_v0.1.md)、approved bundle、approval record 和 `stage5d_governance.py` 精确绑定全部 48 项、Stage 5A approved identity 与 Stage 5C commit `7f64c584c5c7be5e2385a177fab9e5d31e3f665b`。当前 5D-1 实施按[第一条预注册订单/合同历史回放](docs/validation/stage5d-first-order-contract-replay-preregistration.md)收口：case 为 `stage4_4a4_case_001` 的匿名合成 ENTER/BUY，期间固定为 `2025-01-20T02:10:00Z` 至 `2025-01-21T07:01:00Z`，只允许 opening、buy、现金结算、证券交收、可卖五类事件；期末 mark 固定为 `8`，期末 NAV `99986.77`，总 P&L `-13.23`。其他规则保持批准但未实现，并对越界输入显式失败关闭。`5D-2` 仍为 `currently_authorized=false`。
...
3. Stage 5A 四十项和 Stage 5D 四十八项均已按各自 exact bundle 获批，5B 市场/成交与 5C 组合/内存账本已完成 scope-limited 合成验收；第一条订单/合同历史 case、horizon、五事件 inventory、支持矩阵及 `-13.23` P&L golden 已预注册。下一步只实现 opening rematerialization、期末 mark/NAV、受限 P&L 和 complete replay；未命中的 5D 会计边角不提前实现，命中但不支持的事件必须 `BLOCKED/ABSTAIN`。受限 5D-1 独立验收后才讨论 5D-2。
