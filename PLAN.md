...
| Stage 6 | `in_progress / 6B offline implementation accepted` | 历史验证与冠军挑战 | 6A 35 项与 6B 32 项均已原子批准；6B contracts、issuer、provider adapter、隔离临时库原子 seal 和 failure matrix 已验收，仍缺新的短期只读凭据完成真实 HTTPS validation-only seal；正式历史 run、正式 migration 与 6C—6D 未授权 |
...
状态：`in_progress / 6B offline implementation accepted；live HTTPS validation-only seal pending`
...
| Stage 6 | 6A/6B exact 规则与批准谱系；[6B 离线实现验收](docs/validation/stage6b-historical-admission-offline-acceptance.md)；contracts/issuer/provider adapter；隔离临时库原子 seal；failure matrix；`1014 passed, 4 skipped` | 新短期只读凭据下的真实 HTTPS validation-only confirmation/seal；正式 migration、完整 preregistration、6C/6D 仍须独立批准 | `in_progress / 6B offline implementation accepted` |
...
3. Stage 6A/6B 规则治理和 6B 离线 admission/seal 实现已经完成；下一步只是在新的短期只读凭据下，对真实完整 Release closure 执行一次隔离的 HTTPS validation-only confirmation/seal。该验收完成前不得进入正式 migration、正式 preregistration 或 6C；也不得回到 Stage 5D 扩张会计覆盖来绕过此门。
