...
- [产业卡点及事件驱动系统 PRD v0.3](产业卡点及事件驱动系统/01_需求/产业卡点及事件驱动系统_PRD_v0.3.md) 已于 `2026-07-31` 获用户批准并 `supersedes` v0.2；其原始字节保持不变。[PRD v0.4 边界修订补充](产业卡点及事件驱动系统/01_需求/产业卡点及事件驱动系统_PRD_v0.4边界修订补充.md)已通过[独立批准记录](产业卡点及事件驱动系统/01_需求/产业卡点及事件驱动系统_PRD_v0.4边界修订批准记录.md)原子批准八项，只修订 `StrategyInputRef` 所有权、Manifest→投影顺序、对象表和 C1 双门。首个最小订单/合同规则包已在 `stage2b_synthetic_validation` scope 内批准并实现，Stage 4 的 14 项 P0 规则也已分四批批准并实现局部 evaluator；v0.2 只作历史追溯。
...
| `v3.32` | 2026-08-28 | `active` | 形成产业 PRD v0.4 边界修订补充草案：保持 v0.3 字节不变，只更正 `StrategyInputRef` 由 IS 从验证后的 KB ReleaseReference 构造、Manifest 校验顺序、对象所有权和 C1a/C1b 双完成门；`PRD04-BND-01—08` 全部 pending，旧 Stage 6B 和 spike 不在范围。 |
| `v3.33` | 2026-08-28 | `active` | 按 owner 条件授权，在确认无阻塞 P0/P1 后原子批准 PRD v0.4 边界补充 `PRD04-BND-01—08`；v0.3/pending draft 字节保持不变，批准只使四处边界修订生效，不授权 Stage 6B 重构、spike 清理、数据、repin、运行或交易。 |
