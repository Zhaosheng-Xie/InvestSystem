...
| Stage 6 | `in_progress / 6A—6B governance approved` | 历史验证与冠军挑战 | 6A 35 项与 6B 32 项均已原子批准；6B capability 只授权隔离 validation-only admission 实现/验收，runtime 尚未完成；正式历史 run、正式 migration 与 6C—6D 未授权 |
...
状态：`in_progress / 6A—6B governance approved；6B implementation pending`
...
3. Stage 5A 四十项和 Stage 5D 四十八项均已按各自 exact bundle 获批，5B 市场/成交、5C 组合/内存账本及受限 5D-1 第一条订单/合同 complete replay 均已完成 scope-limited 合成验收。下一步不是继续局部扩张 5D 会计覆盖，而是由 owner 决定进入 Stage 6 的正式历史验证设计，或另行形成 5D-2 持久化授权；两者都不能从当前 synthetic capability 自动继承权限。
