...
- [产业卡点及事件驱动系统 PRD v0.3](产业卡点及事件驱动系统/01_需求/产业卡点及事件驱动系统_PRD_v0.3.md) 已于 `2026-07-31` 获用户批准并 `supersedes` v0.2；其原始字节保持不变。[PRD v0.4 边界修订补充草案](产业卡点及事件驱动系统/01_需求/产业卡点及事件驱动系统_PRD_v0.4边界修订补充.md)只修订 `StrategyInputRef` 所有权、Manifest→投影顺序、对象表和 C1 双门，当前八项决定全部 pending。首个最小订单/合同规则包已在 `stage2b_synthetic_validation` scope 内批准并实现，Stage 4 的 14 项 P0 规则也已分四批批准并实现局部 evaluator；v0.2 只作历史追溯。
