"""Draft-only governance for Stage 4 / 4A-4 semantics.

The checked-in expectation, valuation, and exit bundle is an owner-review
proposal. It grants no runtime or trading authority, exposes no evaluator, and
cannot issue an :class:`ApprovedRuleCapability`.
"""
...
STAGE4_4A4_DRAFT_RULE_BUNDLE_ID = "industrial_event_stage4_4a4_expectation_valuation_exit"
STAGE4_4A4_DRAFT_RULE_BUNDLE_VERSION = "0.1.0-draft"
STAGE4_4A4_DRAFT_RULE_BUNDLE_SHA256 = (
    "2d6ebeafeb93fd0d799ab31c1a93e88639e7979fc6416179a18158a9a4450055"
)
STAGE4_4A4_DRAFT_RULES_SHA256 = "5692fae5ab76c233d2f3d3ff3c0e23002062a179426de843c46d5439c09d543c"
STAGE4_4A4_DRAFT_SPECIFICATION_SHA256 = (
    "7f2f1238ff5d9bae1c7a96b212b87dd56a04ac8a9013715e46d2b6cc9d864a62"
)
...
STAGE4_4A4_OWNER_APPROVAL_ITEM_IDS = tuple(f"4A4-APPROVAL-{index:03d}" for index in range(1, 25))
...
class Stage4ExpectationValuationExitRuleProposalError(ValueError):
    """Stable fail-closed rejection for a 4A-4 draft proposal."""
...
def _strict_mapping(
    ...
    *,
    ...
    if actual != keys:
        ...
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_STRUCTURE_DRIFT",
...
class Stage4ExpectationValuationExitRuleProposal(CanonicalModel):
    """Verified identity of the non-executable 4A-4 owner-review proposal."""

    bundle_hash: HashDigest
    rules_hash: HashDigest
    specification_hash: HashDigest
    requirement_ids: tuple[str, ...] = field(default=STAGE4_4A4_REQUIREMENT_IDS, init=False)
    pending_owner_approval_item_ids: tuple[str, ...] = field(
        default=STAGE4_4A4_OWNER_APPROVAL_ITEM_IDS,
        init=False,
    )
...
def stage4_expectation_valuation_exit_rule_proposal_from_document(
    document: RuleBundleDocument,
) -> Stage4ExpectationValuationExitRuleProposal:
    """Verify the exact 4A-4 draft proposal without granting authority."""
    ...
    if (document.strategy_id, document.bundle_id, document.bundle_version) != expected_identity:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_IDENTITY_UNSUPPORTED",
            "the document is not the pinned 4A-4 owner-review proposal",
        )
    if document.declared_status is not RuleStatus.DRAFT:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_STATUS_UNSUPPORTED",
            "4A-4 remains draft until all owner decisions are explicitly approved",
        )
    if document.bundle_hash().value != STAGE4_4A4_DRAFT_RULE_BUNDLE_SHA256:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_BUNDLE_HASH_DRIFT",
            "the complete draft bundle differs from the reviewed proposal",
        )
    if canonical_sha256(document.rules) != STAGE4_4A4_DRAFT_RULES_SHA256:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_RULES_HASH_DRIFT",
            "the draft machine semantics differ from the pinned proposal",
        )

    rules = _strict_mapping(document.rules, keys=_RULE_KEYS, field_name="4A-4 rules")
    ...
    if rules["authorization_boundary"] != expected_boundary:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_AUTHORITY_DRIFT",
    ...
    expected_batch = freeze_json(
        {
            "batch_id": "4A-4",
            "requirement_ids": list(STAGE4_4A4_REQUIREMENT_IDS),
            "owner_approval_required": True,
            "approval_items_required": 24,
            "approved_items": 0,
        }
    )
    if rules["batch"] != expected_batch:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_APPROVAL_STATE_DRIFT",
            "the proposal must retain all twenty-four pending owner decisions",
        )
    ...
    expected_dependencies = freeze_json(
        {
            ...
            "4A-3": {
                "bundle_id": "industrial_event_stage4_4a3_gate_profit_scenarios",
                "bundle_version": "0.1.0",
                "bundle_hash": ("e6936e9c236fd7ed3a67eb8c5e01cb02d23d8fa20c8fcd7a3ccbd615220619b2"),
            },
        }
    )
    if rules["approved_upstream_dependencies"] != expected_dependencies:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_UPSTREAM_DEPENDENCY_DRIFT",
            "4A-4 must bind the exact approved 4A-1 through 4A-3 bundles",
        )

    modules = rules["rule_modules"]
    if not isinstance(modules, Mapping) or set(modules) != set(STAGE4_4A4_REQUIREMENT_IDS):
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_RULE_SET_DRIFT",
            "the proposal must contain exactly FR-GATE-004/005 and FR-EXIT-001",
        )
    for requirement_id in STAGE4_4A4_REQUIREMENT_IDS:
        module = _strict_mapping(
            modules[requirement_id],
            keys=_MODULE_KEYS,
            field_name=f"4A-4 module {requirement_id}",
        )
        if module["requirement_id"] != requirement_id or module["status"] != "draft":
            raise Stage4ExpectationValuationExitRuleProposalError(
                "STAGE4_4A4_DRAFT_MODULE_STATUS_DRIFT",
                f"{requirement_id} must remain an identity-bound draft",
            )
        if not isinstance(module["semantics"], Mapping) or not module["semantics"]:
            raise Stage4ExpectationValuationExitRuleProposalError(
                "STAGE4_4A4_DRAFT_MODULE_EMPTY",
                f"{requirement_id} must contain reviewable proposed semantics",
            )

    raw_approval_items = rules["owner_approval_items"]
    if not isinstance(raw_approval_items, tuple) or len(raw_approval_items) != 24:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_APPROVAL_ITEMS_DRIFT",
            "the proposal must contain exactly twenty-four pending approval items",
        )
    ...
    for index, raw_item in enumerate(raw_approval_items):
        item = _strict_mapping(
            ...
            field_name=f"4A-4 owner approval item {index}",
        )
        if item["status"] != "pending":
            raise Stage4ExpectationValuationExitRuleProposalError(
                "STAGE4_4A4_DRAFT_APPROVAL_ITEM_PREAPPROVED",
    ...
    if tuple(actual_approval_ids) != STAGE4_4A4_OWNER_APPROVAL_ITEM_IDS:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_APPROVAL_ITEMS_DRIFT",
    ...
    expected_binding = freeze_json(
        {
            "path": STAGE4_4A4_DRAFT_SPECIFICATION_PATH,
            "hash": {
                "algorithm": "sha256",
                "value": STAGE4_4A4_DRAFT_SPECIFICATION_SHA256,
            },
    ...
    if rules["document_binding"] != expected_binding:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_SPECIFICATION_BINDING_DRIFT",
            "the machine proposal does not bind the exact owner-review document",
        )
    if rules["runtime_must_not_parse_markdown"] is not True:
        raise Stage4ExpectationValuationExitRuleProposalError(
            "STAGE4_4A4_DRAFT_MARKDOWN_RUNTIME_BOUNDARY_DRIFT",
            "runtime code must never parse Markdown as strategy semantics",
        )

    return Stage4ExpectationValuationExitRuleProposal(
        bundle_hash=document.bundle_hash(),
        rules_hash=HashDigest(algorithm="sha256", value=STAGE4_4A4_DRAFT_RULES_SHA256),
        specification_hash=HashDigest(
            algorithm="sha256",
            value=STAGE4_4A4_DRAFT_SPECIFICATION_SHA256,
        ),
