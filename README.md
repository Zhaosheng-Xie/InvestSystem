...
- [InvestSystem 实施计划 v3.13](PLAN.md)：本仓库唯一正式实施路线图；当前为 `Stage 0—2B completed / Stage 3 completed_with_scope_limits / Stage 4 completed_with_scope_limits / Stage 5B—5D-1 completed_with_scope_limits / Stage 6B completed_with_scope_limits / Stage 6C v0.2 draft pending owner approval`。6B 的离线实现和[真实 HTTPS validation-only seal](docs/validation/stage6b-live-https-acceptance.md)均已验收；[6C v0.2 精确待批准草案](产业卡点及事件驱动系统/03_规则与规格/Stage6_6C开发样本WalkForward与冠军挑战精确规则包_v0.2.md)已关闭 holdout、统计、bootstrap、coverage 和 TWR 五项方法风险。该草案没有 capability、evaluator 或 holdout 权限；正式状态库迁移、historical run、6C 执行和 6D holdout 均未授权。Stage 5D 仍只覆盖固定 ENTER/BUY case，完整生产策略仍不存在。
