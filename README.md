...
- [InvestSystem 实施计划 v1.7](PLAN.md)：本仓库唯一正式实施路线图；PLAN v0.4 为批准基线，当前为 `Stage 0—2B completed / Stage 3 deferred by owner / Stage 4 in progress`。Stage 3 的完成门仍保留并在正式历史验证前补齐；Stage 4 的 4A-1 与 4A-2 已在各自精确批准的合成 research-validation scope 内实现，4A-3 已形成待批准精确 draft，完整策略仍未实现。
...
Stage 2 进入 Stage 4 的[复核](docs/validation/stage2-reentry-audit.md)已通过。owner 于 `2026-08-03` 决定本轮跳过 Stage 3、启动 Stage 4；Stage 3 标为 `deferred` 而非完成。Stage 4 当前处于 [4A rule governance](docs/validation/stage4-development-status.md)：14 项 P0 inventory 中 4A-1 与 4A-2 共八项已批准并实现局部 evaluator；4A-3 的 `FR-GATE-001—003` 已形成 20 项待批准决策和零权限 machine proposal，没有 approval record 或 evaluator；4A-4 三项仍未形成精确提案。因此完整 Stage 4 runtime capability 继续关闭。
