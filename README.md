...
- [InvestSystem 实施计划 v2.1](PLAN.md)：本仓库唯一正式实施路线图；PLAN v0.4 为批准基线，当前为 `Stage 0—2B completed / Stage 3 in progress / Stage 4 in progress`。Stage 3A 已完成离线 Client/export 验收；Stage 3B 已把传输快照重固定到 KB `aab36fe`，并通过独立进程、只读凭据的正式本机 HTTP 验收。3C tcloud 与 3D 正式 Context Pack 策略 smoke 尚未开始，所有 3B 输出仍为 `authority_eligible=false`。Stage 4 的 4A-1—4A-3 已在各自精确批准的合成 research-validation scope 内实现，4A-4 仍是零权限待批准草案，完整策略仍未实现。
