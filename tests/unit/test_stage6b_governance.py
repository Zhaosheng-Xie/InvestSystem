...
SPECIFICATION_PATH = RULE_DIRECTORY / "Stage6_6B历史准入状态确认与原子留存精确规则包_v0.1.md"
APPROVAL_DOCUMENT_PATH = RULE_DIRECTORY / "Stage6_6B历史准入状态确认与原子留存批准记录_v0.1.md"
DRAFT_PATH = (
    MACHINE_DIRECTORY / "industrial_event_stage6_6b_historical_admission_atomic_retention_"
...
APPROVAL_RAW_SHA256 = "a1a28288a13245ad7cd420033a6a11f64906c97498b8451a0fe07675d6abbc3d"
...
def test_checked_in_stage6b_approval_lineage_is_exact(repository_root: Path) -> None:
    document, approval = _artifacts(repository_root)

    assert document.declared_status is RuleStatus.APPROVED
    assert document.bundle_id == STAGE6_6B_RULE_BUNDLE_ID
    assert document.bundle_version == STAGE6_6B_RULE_BUNDLE_VERSION
    assert sha256((repository_root / APPROVED_PATH).read_bytes()).hexdigest() == (
        STAGE6_6B_RULE_BUNDLE_RAW_SHA256
    )
    assert document.bundle_hash().value == STAGE6_6B_RULE_BUNDLE_SHA256
    assert canonical_sha256(document.rules) == STAGE6_6B_RULES_SHA256
    assert approval.approval_id == STAGE6_6B_RULE_APPROVAL_ID
    assert approval.bundle_hash == document.bundle_hash()
    assert approval.approval_scope is STAGE6_6B_APPROVAL_SCOPE
    assert approval.canonical_sha256() == STAGE6_6B_RULE_APPROVAL_RECORD_SHA256
    ...
    approval_items = [
        ...
        .splitlines()
        if line.startswith("- [x] `6B-")
    ]
    assert len(specification_items) == len(approval_items) == 32
    ...
    assert sha256(draft_path.read_bytes()).hexdigest() == STAGE6_6B_DRAFT_RAW_SHA256
    assert draft.bundle_hash().value == STAGE6_6B_DRAFT_BUNDLE_SHA256
    assert canonical_sha256(draft.rules) == STAGE6_6B_DRAFT_RULES_SHA256
    assert canonical_sha256(draft.rules["owner_approval_items"]) == (
        STAGE6_6B_DRAFT_OWNER_ITEMS_SHA256
    )


def test_stage6b_capability_is_isolated_validation_only(repository_root: Path) -> None:
    document, approval = _artifacts(repository_root)
    ...
    assert capability.approval_scope is RuleApprovalScope.STAGE6_HISTORICAL_ADMISSION_VALIDATION
    assert capability.approval_id == STAGE6_6B_RULE_APPROVAL_ID
    ...
    assert boundary["authorizes_validation_only_public_https_read"] is True
    assert boundary["authorizes_validation_only_confirmation"] is True
    assert boundary["authorizes_validation_only_seal"] is True
    assert boundary["authority_eligible"] is False
    assert all(
        boundary[field] is False
        for field in (
            "authorizes_formal_historical_run",
            "authorizes_strategy_evaluator",
            "authorizes_development",
            "authorizes_walk_forward",
...
def test_stage6b_default_registry_and_other_stage_scope_fail_closed(
    repository_root: Path,
) -> None:
    document, approval = _artifacts(repository_root)
    with pytest.raises(ValueError, match="RULE_BUNDLE_IDENTITY_NOT_APPROVED"):
        require_stage6b_admission_validation_capability(document)

    wrong_scope = replace(
        approval,
        approval_scope=RuleApprovalScope.STAGE6_HISTORICAL_VALIDATION_GOVERNANCE,
    )
    with pytest.raises(Stage6BApprovalCompatibilityError) as exc_info:
        require_stage6b_admission_validation_capability(
            document,
            registry=RuleApprovalRegistry((wrong_scope,)),
        )
    assert exc_info.value.code == "STAGE6B_APPROVAL_SCOPE_MISMATCH"


def test_stage6b_partial_items_or_phase_expansion_fails_closed(repository_root: Path) -> None:
    document, approval = _artifacts(repository_root)

    changed_items = _changed_document(document, lambda rules: rules["owner_approval_items"].pop())
    matching_items = replace(approval, bundle_hash=changed_items.bundle_hash())
    with pytest.raises(Stage6BApprovalCompatibilityError) as items_exc:
        require_stage6b_admission_validation_capability(
            changed_items,
            registry=RuleApprovalRegistry((matching_items,)),
        )
    assert items_exc.value.code == "STAGE6B_APPROVAL_ITEMS_INVALID"

    changed_phase = _changed_document(
        document,
        lambda rules: rules["phase_authority"].update({"6C": "authorized"}),
    )
    matching_phase = replace(approval, bundle_hash=changed_phase.bundle_hash())
    with pytest.raises(Stage6BApprovalCompatibilityError) as phase_exc:
        require_stage6b_admission_validation_capability(
            changed_phase,
            registry=RuleApprovalRegistry((matching_phase,)),
        )
    assert phase_exc.value.code == "STAGE6B_PHASE_AUTHORITY_MISMATCH"


def test_stage6b_formal_historical_migration_or_trading_expansion_fails_closed(
    repository_root: Path,
) -> None:
    document, approval = _artifacts(repository_root)
    ...
    for field_name in (
        ...
        assert exc_info.value.code == "STAGE6B_AUTHORIZATION_BOUNDARY_MISMATCH"


def test_stage6b_draft_source_and_semantic_guard_drift_fail_closed(
    repository_root: Path,
) -> None:
    document, approval = _artifacts(repository_root)
    ...
    with pytest.raises(Stage6BApprovalCompatibilityError) as source_exc:
        require_stage6b_admission_validation_capability(
            changed_source,
            registry=RuleApprovalRegistry((matching_source,)),
        )
    assert source_exc.value.code == "STAGE6B_APPROVAL_SOURCE_MISMATCH"

    value = document.to_json_value()
    value["rules"]["approved_semantic_guards"]["authority_contract_hash"] = "0" * 64
    ...
    with pytest.raises(Stage6BApprovalCompatibilityError) as guard_exc:
        require_stage6b_admission_validation_capability(
            changed_guard,
            registry=RuleApprovalRegistry((matching_guard,)),
        )
    assert guard_exc.value.code == "STAGE6B_SEMANTIC_GUARD_MISMATCH"

    guards = document.rules["approved_semantic_guards"]
    assert isinstance(guards, Mapping)
    assert guards["authority_contract_hash"] == STAGE6_6B_AUTHORITY_CONTRACT_SHA256


def test_stage6b_owner_item_sequence_and_approved_hash_are_exact(repository_root: Path) -> None:
    raw = _json(repository_root / APPROVED_PATH)
    items = raw["rules"]["owner_approval_items"]

    assert tuple(item["approval_item_id"] for item in items) == STAGE6_6B_OWNER_APPROVAL_ITEM_IDS
    assert {item["status"] for item in items} == {"approved"}
    assert canonical_sha256(items) == STAGE6_6B_OWNER_APPROVAL_ITEMS_SHA256
    ...
    assert exc_info.value.code == "STAGE6B_APPROVAL_ITEMS_INVALID"
