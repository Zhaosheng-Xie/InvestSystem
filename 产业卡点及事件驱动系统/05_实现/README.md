...
需求与边界已于 Stage 0 冻结；Stage 1 的工程与机器契约骨架、Stage 2A 的离线 Release 消费与准入内核，以及 Stage 2B 的最小合成策略纵向切片均已通过根级正式验收。owner 于 `2026-08-08` 恢复 Stage 3：3A 完成 HTTP Client、immutable export 和官方 fixture 离线验收；3B 重固定 KB `aab36fe` 公共传输契约并通过独立 KB RC HTTP 验收；3C 通过真实 tcloud `market_daily` 传输验收；3D 已通过正式 Context Pack/Evidence Release，完成 provider-neutral 映射和 validation-only `ABSTAIN` smoke。owner 于 `2026-08-12` 按无 run authority 的边界正式关闭 Stage 3；确认和持久化准入移交 Stage 6/7。Stage 4 的 4A-1 已实现上下文、历史语义、产业卡点与受益公司映射；4A-2 已实现事件状态、E4、主体/PIT 与审计分层；4A-3 已实现 Gate 1—2、利润分母和四情景；4A-4 已实现 Gate 3—4、估值和退出；4B 已实现从四批原始输入重新计算的完整合成编排。Stage 4 批准自身不授权 backtest、paper、shadow、live、仓位、组合、成交、P&L 或订单；Stage 5 只在下述精确匿名合成范围内扩展验证能力。
...
Stage 5D 的[48 项精确规则](../03_规则与规格/Stage5_5D公司行动估值P&L完整回放与原子持久化精确规则包_v0.1.md)已被 owner 整包原子批准；原 draft 保持不变，独立批准记录、approved bundle、approval record 和 `stage5d_governance.py` fail-closed verifier 已形成。受限 `5D-1` 已按[正式验收](../../docs/validation/stage5d-first-order-contract-replay-acceptance.md)完成第一条预注册 ENTER/BUY 历史回放实际需要的同次 Stage 5C 重算、source-driven Ledger V2、结算/可用性、mark/NAV、十八格 P&L 与 complete/audit replay。它不因治理清单完整就声称全部证券会计边角已经实现；支持矩阵外输入仍稳定 `BLOCKED/ABSTAIN` 且不得发布 partial NAV/P&L。`5D-2` SQLite v4 原子持久化继续保持 `currently_authorized=false`，不得提前新增数据库表或 migration，也不得改变 Stage 5C 的 scope-limited 契约。

[第一条订单/合同回放预注册](../../docs/validation/stage5d-first-order-contract-replay-preregistration.md)固定了匿名合成 ENTER/BUY、五个金融事件、价格不变的期末 mark 以及 `-13.23` 总 P&L。`stage5d_bounded_replay.py` 现已实现 opening rematerialization、两个 inclusive prefix、期末 mark/NAV、十八格 P&L 和 deterministic complete/audit replay；SELL、公司行动、外部现金流与持久化继续不进入本切片。
...
Stage 3B 的精确重固定、独立进程 HTTP 输出和脱敏验证证据见根级 [Stage 3B 正式跨仓只读 HTTP 验收](../../docs/validation/stage3b-http-acceptance.md)。[Stage 3C](../../docs/validation/stage3c-tcloud-http-acceptance.md)关闭真实 tcloud `market_daily` 传输，[Stage 3D](../../docs/validation/stage3d-context-pack-http-acceptance.md)关闭正式 Context Pack/Evidence 获取、provider-neutral 映射和 validation-only `ABSTAIN` smoke。三者均不代替 CAS/Observation 持久化或 run-scoped authority。
...
- 已完成契约轨：`KB 官方 fixture → 离线 validator/projector → ArtifactConsumptionReceipt + Observations + ReleaseRetentionClosure → provider-neutral DTO`；Stage 3A 已完成只读 Client/export 离线能力，Stage 3B 已完成独立进程 HTTP 兼容验收；
- 已完成正式传输核验：`精确 market-daily Published Release → Manifest/制品/状态/权限/Receipt 验证`，不生成订单/合同策略结论；
- 已完成 validation-only 汇合：`精确 Context Pack/Evidence Published Release → provider-neutral 输入 → StrategyRunManifest → 真实只读 smoke（ABSTAIN）`；CAS/Observation 持久化、确认和真实 authority 仍未授权。
