...
需求与边界已于 Stage 0 冻结；Stage 1 的工程与机器契约骨架、Stage 2A 的离线 Release 消费与准入内核，以及 Stage 2B 的最小合成策略纵向切片均已通过根级正式验收。owner 于 `2026-08-03` 决定跳过 Stage 3 并启动 Stage 4；Stage 3 保持 `deferred`。Stage 4 的 4A-1 已实现上下文、历史语义、产业卡点与受益公司映射；4A-2 已实现事件状态、E4、主体/PIT 与审计分层；4A-3 当前只固定待批准的 Gate 1—2、利润分母和四情景提案，没有 Gate evaluator。当前批准不授权 backtest、paper、shadow、live、仓位或订单。

Stage 2B 的实际交付、验证锚点和未授权范围见根级 [Stage 2B 正式验收记录](../../docs/validation/stage2b-acceptance.md)。

Stage 4 当前交付见[开发状态](../../docs/validation/stage4-development-status.md)：完整 14 项 P0 inventory 中前八项已批准。`stage4_context_industry.py` 和 `stage4_event_semantics.py` 分别只接受精确 4A-1/4A-2 approved bundle/approval capability；`stage4_event_governance.py` 继续固定 4A-2 原 draft 谱系。新增 `stage4_gate_governance.py` 只验证 4A-3 draft 的精确 identity/hash、20 项 pending 决策、已批准 4A-1/4A-2 上游 bundle 和零权限，刻意不提供 Gate evaluator 或 capability issuer。其余六项仍为 `draft`；只有 14 项全部批准、四类测试齐全、inventory hash 与完整 machine bundle 精确绑定且零交易权限未漂移时，`require_stage4_rule_capability` 才可能签发完整 capability。
