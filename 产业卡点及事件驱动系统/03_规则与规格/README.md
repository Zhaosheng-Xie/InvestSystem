...
Stage 4 已完成前两个执行批次，完整 capability 仍关闭：
...
- `stage4_synthetic_research_validation` 是独立 scope，Stage 2B capability 不能复用；4A-1 与 4A-2 只允许各自的局部合成验证，均不能签发完整 Stage 4 runtime capability。
