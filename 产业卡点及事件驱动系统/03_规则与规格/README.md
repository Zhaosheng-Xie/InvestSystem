...
Stage 4 已完成前两个执行批次，并形成第三批待批准提案；完整 capability 仍关闭：

- [Stage 4 完整 P0 规则清单与批准包 v0.1](Stage4完整P0规则清单与批准包_v0.1.md)：`partially_approved / 4A-1 and 4A-2 completed`；
...
- [4A-3 四道门、利润分母与情景规则包](Stage4_4A3四道门利润分母与情景规则包_v0.1.md)：`draft_for_owner_approval`，含 20 项待确认决策；[draft machine proposal](机器制品/industrial_event_stage4_4a3_gate_profit_scenarios_v0.1.0-draft.rule-bundle.json)只固定提案、4A-1/4A-2 上游依赖和零权限，不是 runtime rule bundle；
- [Stage 4 P0 machine inventory](机器制品/industrial_event_stage4_p0_rule_inventory_v0.1.0-draft.json)：14 项完整清单，前八项 `approved`，其余六项 `draft`；
- `stage4_synthetic_research_validation` 是独立 scope，Stage 2B capability 不能复用；4A-1 与 4A-2 只允许各自的局部合成验证，4A-3 当前没有 capability/evaluator，均不能签发完整 Stage 4 runtime capability。
