...
Stage 4 的 4A governance、4A-1 合成执行和 4A-2 draft 治理测试验证：
...
- 4A-2 draft 精确绑定文字规格、canonical bundle/rules hash 和 16 项 `pending` owner 决策；默认 registry、伪造的 matching approval、语义漂移和自声明 `approved` 均失败关闭；当前没有 4A-2 approval record 或事件 evaluator。

这些测试只证明 4A-1 在精确批准的合成 research-validation scope 内可执行，并证明 4A-2 仍是一份不可执行的确定 draft；不证明 4A-2 业务语义已批准、完整 Stage 4、历史有效性或交易能力已经实现。
